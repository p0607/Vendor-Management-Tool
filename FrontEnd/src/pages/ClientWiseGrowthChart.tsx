...
function normalizeToFullMonthName(monthName: any): string {
  if (!monthName) return '';
...
function getMonthNumber(monthName: string): number {
  ...
  return months[normalizeToFullMonthName(monthName)] || 0;
}

function getMonthKey(month: string, year: number): string {
...
export default function ClientWiseGrowthChart() {
  ...
  const [periodValue, setPeriodValue] = useState<string>(String(getCurrentFinancialYear()));
  ...
  const periodLabelsAndKeys = useMemo(() => {
    ...
    if (periodType === 'year' && periodValue) {
      ...
      if (match && yearMatch) {
        ...
        const displayYear = (m: number) => (q === 4 && m <= 3) ? y + 1 : y;
        months.forEach((m) => {
          const yr = displayYear(m);
          result.push({ key: `${yr}-${String(m).padStart(2, '0')}`, label: `${MONTH_ABBR[m]} ${yr}` });
        });
      }
    ...
    if (selectedClient === '__all__') {
      ...
      clientNames.forEach((clientName) => {
        ...
        filteredByBU.forEach((item: any) => {
          ...
          if (!item.month || item.year == null) return;
          const monthName = normalizeToFullMonthName(item.month);
          const key = getMonthKey(monthName, item.year);
  ...
  useEffect(() => {
    if (periodType === 'year') setPeriodValue(String(getCurrentFinancialYear()));
        ...
        <div className="client-growth-chart-field">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setPeriodType(v);
              if (v === 'year') setPeriodValue(String(getCurrentFinancialYear()));
