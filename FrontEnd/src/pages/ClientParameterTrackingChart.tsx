...
interface ClientParameterTrackingChartProps {
  ...
  // Add data from ClientMFSCompare
  data?: any[];
  availableParameters?: string[];
}

const ClientParameterTrackingChart: React.FC<ClientParameterTrackingChartProps> = ({
  ...
  // Fetch data from database - using team-report API instead of team-summary-report
  const fetchDataFromDatabase = async () => {
    ...
    try {
      ...
      // Use team-report API instead of team-summary-report
      ...
      const convertedData = res.data.map((item: any) => {
        ...
        // List of fields available in team-report (more fields than team-summary-report)
        const numericFields = [
          'hc', 'salary_cost', 'revenue', 'gpm', 'gpm_percentage', 
          'leave_encashment', 'team_cost', 'opr_cost', 'funding_cost', 
          'np', 'np_percentage', 'rebate', 'passthrough'
        ];
        ...
        // Map field names for compatibility (team-report has np, also map to net_margin)
      ...
      console.error("❌ Error fetching data from team-report API:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const processDataForTimeline = (data: any[]) => {
    ...
    const processedData = filteredData.map(item => {
      ...
      return {
        ...
        netMargin: item.net_margin || item.np || 0,
        gpm: item.gpm || 0,
        hc: item.hc || 0,
        teamCost: item.team_cost || 0,
        salaryCost: item.salary_cost || 0,
        oprCost: item.opr_cost || 0,
        fundingCost: item.funding_cost || 0,
        leaveEncashment: item.leave_encashment || 0,
        gpmPercentage: item.gpm_percentage || 0,
        npPercentage: item.np_percentage || 0,
        rebate: item.rebate || 0,
        passthrough: item.passthrough || 0,
    ...
    if (selectedTimeline !== 'month') {
      const groupedData = processedData.reduce((acc, item) => {
        ...
        acc[item.timelineLabel].salaryCost += item.salaryCost;
        acc[item.timelineLabel].oprCost += item.oprCost;
        acc[item.timelineLabel].fundingCost += item.fundingCost;
        acc[item.timelineLabel].leaveEncashment += item.leaveEncashment;
        acc[item.timelineLabel].rebate += item.rebate;
        acc[item.timelineLabel].passthrough += item.passthrough;
      ...
      const aggregatedData = Object.values(groupedData).map((group: any) => ({
        ...
        salaryCost: group.salaryCost,
        oprCost: group.oprCost,
        fundingCost: group.fundingCost,
        leaveEncashment: group.leaveEncashment,
        gpmPercentage: group.gpmPercentage / group.count,
        npPercentage: group.npPercentage / group.count,
        rebate: group.rebate,
  ...
  useEffect(() => {
    ...
    // Check if chart container exists - use unique ID for client parameter tracking
    const chartContainer = document.getElementById("clientParameterTrackingChart");
    if (!chartContainer) {
      return;
    }
    ...
    am5.array.each(am5.registry.rootElements, (root) => {
      if (root && root.dom && root.dom.id === "clientParameterTrackingChart") root.dispose();
    });

    const root = am5.Root.new("clientParameterTrackingChart");
    ...
    const getParameterFormat = (param: string) => {
      if (param === 'GPM %' || param === 'NP %' || param === 'Net Margin %' || param === 'Growth Margin %') {
        return {
          prefix: '',
          suffix: '%',
          format: '#,##0.00'
  ...
  return (
    ...
    >
      ...
      <div id="clientParameterTrackingChart" style={{ width: "100%", height: "300px" }}></div>
    </Card>
  );
};

export default ClientParameterTrackingChart;
